...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # regularization
        self.nn_dropout = nn.Dropout(config.dropout)
...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.fc_1    = nn.Linear(config.n_embd, 4 * config.n_embd, bias=config.bias)
        self.fc_2    = nn.Linear(config.n_embd, 4 * config.n_embd, bias=config.bias)
...
class ModelArgs:
    block_size: int = 1024
    vocab_size: int = 52_000 # GPT-2 vocab_size of 50257, padded up to nearest multiple of 64 for efficiency
    n_layer: int = 12
    n_head: int = 12
    n_embd: int = 768
    dropout: float = 0.0
...
class Transformer(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        # In modeling_esperia.py, inside the forward() method:
